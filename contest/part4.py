...
def find_circle(c):
    ...
    for i in link:
        if c != current and (c == i[0] or c == i[1]) and(current == i[0] or current == i[1]) and i[0] != second and i[1] != second and second != '':
            ...
            seen = set()
            return False
        if c == i[0] and i[1] not in seen and i[1] not in ans and i[1] not in alert:
            if c == current:
                second = i[1]
            tmp_seen = seen
            seen.add(c)
            if find_circle(i[1]):
                ...
                seen = tmp_seen
        if c == i[1] and i[0] not in seen and i[0] not in ans and i[1] not in alert:
            if c == current:
                second = i[0]
            tmp_seen = seen
            seen.add(c)
...
# 100011501001815|慈周寨
alert = set()
ans = ['东风路东环', '内黄', '留固', '慈周寨', '滑县', '白条河', '汤阴', '黄河路东环', '黄河路西环', '中华路西环', '东风路西环', '柏庄西环', '铜冶', '姚村', '林州', '桂林', '水冶', '中华路东环']
for z in ['东风路东环', '内黄', '留固', '慈周寨', '滑县', '白条河', '汤阴', '黄河路东环', '黄河路西环', '中华路西环', '东风路西环', '柏庄西环', '铜冶', '姚村', '林州', '桂林', '水冶']:
    print(z)
    flag = True
    huiju = list()
    seen = set()
