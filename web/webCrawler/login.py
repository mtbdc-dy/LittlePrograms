...
def login_wangluoquanjingkeshihua():
    ...
    cj = web.webCrawler.webcrawler.get_cookie_without_form(url)
    for item in cj:
    ...
    web.webCrawler.webcrawler.post_web_page_ssl(url, form, cookie)
...
def zte_anyservice_uniportal():
    ...
    cj = web.webCrawler.webcrawler.get_cookie_without_form(url) # 这个函数默认输出结果
    for item in cj:
        cookie = item.name + '=' + item.value
    print(cookie)
